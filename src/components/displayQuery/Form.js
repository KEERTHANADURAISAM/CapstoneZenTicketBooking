...
const Form = () => {
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      console.log(values)
      try {
        const response = await axios.post("https://zen-server.onrender.com/create-query", values);
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
        ...
        <label htmlFor="category" className="card-sublable">Category:</label>
        <select
        ...
        >
          <option value="" label=" ---Select Category---" >           
           <DownOutlined className="downoutlined-icon" /></option>
...
<div>
<label htmlFor="language" className="card-sublable"> Prefered Voice Communication Language:</label>
        <select
          className="select-one"
          id="language"
          name="language"
        ...
        >
          <option value="" label="---Select Language---"> <DownOutlined className="downoutlined-icon" />
            </option>
...
<label className="card-lable">Details</label>
        <label htmlFor="title" className="card-sublable"> Query Title</label>
        <input
      ...
      <div>
        ...
        <label className="card-sublable">Till</label>
        <input
        className="formInputs"
          type="time"
          id="tillTime"
          ...
          onChange={formik.handleChange}
          onBlur={formik.handleBlur}
          value={formik.values.tillTime}
        />
        {formik.touched.tillTime && formik.errors.tillTime ? (
          <div className="error">{formik.errors.tillTime}</div>
        ) : null}
      
      </div>
      <div className="querybtn-flex">
      <button type="submit" className="querybtn-cancel">Cancel</button>
